...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene,
        ...
        // 3. Программно создаем  контроллер и интегрируем в него NavigationController
        let viewController = ViewController()
        let navigationControllet = UINavigationController(rootViewController: viewController)

        // 4. Определить созданный нами navigationControllet в качестве rootViewController
        window.rootViewController = navigationControllet

        // 5. Передать наш настроенный window обратно в SceneDelegate и сделать его видимым
        self.window = window
